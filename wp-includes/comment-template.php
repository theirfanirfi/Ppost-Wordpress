...
function comment_form_title( $noreplytext = false, $replytext = false, $linktoparent = true ) {
	...
	if ( false === $noreplytext ) $noreplytext = __( 'Leave a Replyy' );
...
function comment_form( $args = array(), $post_id = null ) {
	...
	$fields   =  array(
		'author' => '<p class="comment-form-author">' . '<label for="author">' . __( 'نام' ) . ( $req ? ' <span class="required">*</span>' : '' ) . '</label> ' .
		            '<input id="author" name="author" class="form-control" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30" maxlength="245"' . $aria_req . $html_req . ' /></p>',
		'email'  => '<p class="comment-form-email"><label for="email">' . __( 'ای میل' ) . ( $req ? ' <span class="required">*</span>' : '' ) . '</label> ' .
		...
		'url'    => '<p class="comment-form-url"><label for="url">' . __( 'ویب سائٹ' ) . '</label> ' .
	...
	$required_text = sprintf( ' ' . __('<br/> <span class="sp">ضروری شعبوں کو نشان لگا دیا گیا ہے %s</span>'), '<span class="required sp" stye="color:red;">*</span>' );
	...
	$defaults = array(
		'fields'               => $fields,
		'comment_field'        => '<p class="comment-form-comment"><label for="comment">' . _x( '', 'noun' ) . '</label><br/> <textarea id="comment" name="comment" cols="45" rows="8" maxlength="65525" aria-required="true" required="required" class="form-control"></textarea></p>',
		/** This filter is documented in wp-includes/link-template.php */
		'must_log_in'          => '<p class="must-log-in">' . sprintf(
		                              /* translators: %s: login URL */
		                              __( 'تبصرہ پوسٹ کرنے کے لیے آپ کو لاگ ان ہونا ضروری ہے     <a href="%s">لاگ ان</a>' ),
		                              wp_login_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) )
		                          ) . '</p>',
		/** This filter is documented in wp-includes/link-template.php */
		'logged_in_as'         => '<p class="logged-in-as">' . sprintf(
		                              /* translators: 1: edit user link, 2: accessibility text, 3: user name, 4: logout URL */
		                              __( '<a style="clear:both;float:right;" href="%4$s">لاگ آوٹ؟</a> .<a style="clear:both;float:right;" href="%1$s" aria-label="%2$s">  کے طور پر لاگ ان %3$s</a>' ),
		                              get_edit_user_link(),
		                              /* translators: %s: user name */
		                              esc_attr( sprintf( __( ' %s کے طور پر لاگ ان . اپنی پروفائل میں ترمیم کریں.' ), $user_identity ) ),
		                              $user_identity,
		                              wp_logout_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) )
		                          ) . '</p>',
		'comment_notes_before' => '<p class="comment-notes sp"><span id="email-notes">' . __( 'آپ کا ای میل پتہ شائع نہیں کیا جائے گا.' ) . '</span>'. ( $req ? $required_text : '' ) . '</p>',
		'comment_notes_after'  => '',
		...
		'class_submit'         => 'submit',
		'name_submit'          => 'submit',
		'title_reply'          => __( 'یہاں خبروں کے بارے میں اپنے خیالات کا اشتراک کریں' ),
		'title_reply_to'       => __( 'کا جواب دیں %s' ),
		'title_reply_before'   => '<h3 id="reply-title" class="comment-reply-title" style="color:#1d744a;">',
		'title_reply_after'    => '</h3>',
		'cancel_reply_before'  => ' <small>',
		'cancel_reply_after'   => '</small>',
		'cancel_reply_link'    => __( '<br/> جواب منسوخ کریں' ),
		'label_submit'         => __( 'شائع کریں' ),
		'submit_button'        => '<input name="%1$s" type="submit" id="%2$s" class="%3$s btn btn-success form-control" value="%4$s" />',
